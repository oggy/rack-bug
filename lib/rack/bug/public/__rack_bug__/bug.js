...
jQuery(function($) {
  ...
  $.extend($.rackBug, {
    init: function() {
      ...
      $('#rack_bug a.show_full_backtrace').click(function() {
        var link = $(this);
        if (link.hasClass('activated')) {
          link.parents('td').find('li.filtered').hide();
          link.removeClass('activated').text('Show Full Backtrace');
        } else {
          link.parents('td').find('li.filtered').show();
          link.addClass('activated').text('Show Filtered Backtrace');
        }
      });
